...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void onRegisterClick(View view){
        if (!isEmptyText(inputNameR) && !isEmptyText(inputLastNameR) && !isEmptyText(inputEmailR) && !isEmptyText(inputPassword)) {
            ...
            Toast.makeText(RegisterActivity.this, "Lütfen tüm alanları giriniz", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void register(String URL){
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                if(!response.isEmpty()){
                    Toast.makeText(RegisterActivity.this, "Doğru şekilde kaydedildi", Toast.LENGTH_SHORT).show();
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("userName", inputNameR.getText().toString());
            ...
            for(NetworkInterface intf : interfaces){
                ...
                for(InetAddress addr : addrs){
                    if(!addr.isLoopbackAddress() && addr instanceof Inet4Address){
                        address = addr.getHostAddress().toUpperCase(new Locale("tr", "TR"));
                    }
                }
            }
        }catch (Exception e){
